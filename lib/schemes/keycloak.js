...
export default class KeycloakScheme {
  constructor (auth, userOptions) {
    ...
    let keycloak_ssl = userOptions.hasOwnProperty('keycloak_ssl') ? userOptions.keycloak_ssl : true
    let protocol = userOptions.protocol || ssl ? 'https' : 'http'
    let keycloak_protocol = userOptions.keycloak_protocol || keycloak_ssl ? 'https' : 'http'
    
    let client_id = userOptions.client_id || 'nuxt-app'
    let realm = userOptions.realm || 'master'
    let base_uri = userOptions.base_uri || `${keycloak_protocol}://${keycloak_host}${keycloak_port}/auth/realms/${realm}/protocol/openid-connect`
    ...
    let account_uri = userOptions.account_uri || `${keycloak_protocol}://${keycloak_host}${keycloak_port}/auth/realms/${realm}/account`
    ...
    let logout_redirect_uri = `${keycloak_protocol}://${keycloak_host}/login`
  ...
  login() {
    ...
    if (process.browser) {      
      if (this.options.use_browser_host) {
        this.updateOptionsForBrowserHostname()
        opts.redirect_uri = this._redirectURI  
      }
  ...
  updateOptionsForBrowserHostname() {    
    if (window && window.location && window.location.hostname) {        
      ...
      const keycloak_protocol = this.options.keycloak_protocol
      const keycloak_port = this.options.keycloak_port
      const realm = this.options.realm
      const nuxt_host = window.location.hostname
      const nuxt_port = browserProtocol === "https" ? "" : this.options.nuxt_port
      const logout_referrer = this.options.logout_referrer
      const client_id = this.options.client_id

      const keycloak_host = `${window.location.hostname}`
      const base_uri = `${keycloak_protocol}://${keycloak_host}${keycloak_port}/auth/realms/${realm}/protocol/openid-connect`
      const account_uri = `${keycloak_protocol}://${keycloak_host}${keycloak_port}/auth/realms/${realm}/account`
