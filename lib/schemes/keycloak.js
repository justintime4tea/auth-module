...
export default class KeycloakScheme {
  ...
  login () {
    ...
    if (process.browser) {      
      let url;
      try {
        //If authorization_endpoint is set in nuxt.config.js, then use that address, otherwise construct from application's host name
        const urlBase = this.options.authorization_endpoint || this.options.base_oid_url.replace(/:\/\/.+?:/, '://' + window.location.hostname + ':') + '/auth'
        url = urlBase + '?' + encodeQuery(opts)
      } catch (e) {
        console.error('Failed to set keycloak host.  Check configuration values.');
      }
